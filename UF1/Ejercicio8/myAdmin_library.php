<?php
	...
	function BaseDatos(){
		...
		$_SESSION["c_id"] = [];
		$_SESSION["c_id_producto"] = [];
		$_SESSION["c_id_usuario"] = [];
		$_SESSION["c_completada"] = [];
		...
		if($resultado->num_rows >= 0){	
			...
			while ($producto = $resultado->fetch_assoc()){
				...
				array_push($_SESSION["p_id_order"], $producto["id_comanda"]);
			}
		}
		...
		//Petición para obtener la información de la comanda
		...
		if($resultado->num_rows >= 0){	
			...
			while ($comanda = $resultado->fetch_assoc()){
				array_push($_SESSION["c_id"], $comanda["id"]);
				array_push($_SESSION["c_id_usuario"], $comanda["id_usuario"]);
				array_push($_SESSION["c_id_producto"], $comanda["id_producto"]);
				array_push($_SESSION["c_completada"], $comanda["completada"]);
			}
		}
		...
		if($resultado->num_rows >= 0){	
			...
			while ($transaccion = $resultado->fetch_assoc()){
				array_push($_SESSION["t_id"], $transaccion["id"]);
				array_push($_SESSION["t_price"], $transaccion["precio"]);
				array_push($_SESSION["t_correcta"], $transaccion["correcta"]);
			}
		}
